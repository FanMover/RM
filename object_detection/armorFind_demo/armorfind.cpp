...
void ArmorFind::process(vector<vector<Point> > contours,const Mat &input,Mat &output,vector<Point> &result,bool ismono){
    ...
    for(int i=0;i<contours.size();i++){
        ...
        if((fabs(RRect.angle) < 45.0 && RRect.size.height > RRect.size.width)
                ...
                RectfirstResult.push_back(RRect); //对轮廓形状进行初筛
        }
    }
    if(RectfirstResult.size() < 2){
        ArmorCenters.clear(); //如果没找到两个或以上，中心值清空
        return;
    }
    sort(RectfirstResult.begin(),RectfirstResult.end(),RotateRectSort); //自定义容器排序算法做初筛
    GetArmorLights(); //得到最后的灯条结果
    sort(RectResults.begin(),RectResults.end(),RotateRectSort); //同一个排序算法排序进一步筛选
    for(int i=0;i<RectResults.size();i++){
        //std::cout<<"("<<RectResults[i].center.x<<","<<RectResults[i].center.y<<")"<<std::endl;
        ellipse(output,RectResults[i],Scalar(255,0,0),2); //椭圆拟合
        /*ostringstream ss;
        ss<<i;
    ...
    GetArmors(output,ismono); //从灯条位置得到装甲中心点，ismono为0表示双目视觉，其他表示单目视觉
    for(int i=0;i<ArmorCenters.size();i++){
        DrawCross(output,ArmorCenters[i],20,Scalar(255,0,255),2); //画出装甲的中心
    }
    result = ArmorCenters; //返回中心值
}
...
void ArmorFind::GetArmorLights(){
    ...
    int cellmaxsize;
    Groups.push_back(RectfirstResult[0]); //单独拿出第一个Rect
    cellmaxsize = RectfirstResult[0].size.height * RectfirstResult[0].size.width; //并将第一个作为最大面积的临时值
    if(cellmaxsize > 2500) cellmaxsize = 0;
    int maxsize;

    for(int i=1;i<size;i++){
        if(RectfirstResult[i].center.x - RectfirstResult[i-1].center.x <10){ //把接近的当做一个群,中心相差小于10
            maxsize = RectfirstResult[i].size.height * RectfirstResult[i].size.width;
            if(maxsize > 2500) continue;
            if(maxsize > cellmaxsize) cellmaxsize = maxsize; //如果下一个Rect大于临时最大，取代
            Groups.push_back(RectfirstResult[i]); //只向容器中添加最后一个
        }else{
            Armorlists.push_back(Groups);  //如果有不接近的，先将之前的一个群当作一个装甲的信息储存起来，再将Groups清空,并赋新值
